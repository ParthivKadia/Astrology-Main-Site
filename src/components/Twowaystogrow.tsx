...
const UI = {
  ...
  card: {
    ...
    radius: "rounded-[16px]",
    innerGap: "gap-5 sm:gap-6 lg:gap-7",
    columns: "lg:grid-cols-[minmax(180px,0.84fr)_minmax(0,1.16fr)]",
  },
  badge: {
    padding: "px-3 py-1.5",
    text: "text-[10px] sm:text-[11px]",
...
const plans = {
  starter: {
    ...
    title: "Starter",
    subtitle: "Mini website + Booking",
    monthly: { price: "₹999", unit: "/month" },
    annually: { price: "₹9,999", unit: "/year" },
    features: [
      "Personal profile & mini website",
      "Services & pricing",
      "Appointment booking",
  ...
  custom: {
    ...
    title: "Custom",
    subtitle: "Fully personalised site + Software",
    monthly: { price: "₹30,000", unit: "/one time" },
    annually: { price: "₹30,000", unit: "/one time" },
    features: [
      "Your own domain (e.g. yourname.com)",
      "Custom design & features",
      "CRM, analytics & integrations",
...
function CheckItem({ label, tint }: { label: string; tint: Tint }) {
  const isLavender = tint === "lavender";
  return (
    <li className="flex items-start gap-2">
      <span
        className={`mt-[1px] flex ${UI.features.iconSize} shrink-0 items-center justify-center rounded-full ${
          isLavender ? "bg-[var(--color-lavender-bg)] text-lavender" : "bg-[var(--color-sage-bg)] text-sage"
        }`}
      >
      ...
      <span className={`min-w-0 leading-[1.4] text-navy ${UI.features.text}`}>{label}</span>
    </li>
  );
}
...
function BrowserPreview({ url, tint, photo, name }: { url: string; tint: Tint; photo: string; name: string }) {
  const panelBg = tint === "lavender" ? "bg-[var(--color-card-lavender)]" : "bg-[var(--color-card-mint)]";

  return (
    <div className="relative w-full">
      <div className={`w-full overflow-hidden border border-border bg-white shadow-card ${UI.browser.radius}`}>
        <div className={`flex items-center gap-1.5 border-b border-border px-3 ${UI.browser.headerHeight}`}>
          <span className="h-[9px] w-[9px] shrink-0 rounded-full bg-[#ff6159]" />
          <span className="h-[9px] w-[9px] shrink-0 rounded-full bg-[#ffbd2e]" />
          <span className="h-[9px] w-[9px] shrink-0 rounded-full bg-[#28c840]" />
          <span className="ml-1.5 min-w-0 truncate text-[9px] text-body sm:text-[10px]">{url}</span>
        </div>

        <div className={`relative flex ${UI.browser.height} items-center overflow-hidden ${UI.browser.padding} ${panelBg}`}>
          <div className="min-w-0 flex-1 pr-2">
            <p className="font-heading text-[15px] font-semibold leading-tight text-navy sm:text-[17px] lg:text-[18px]">{name}</p>
            <p className="mt-1 text-[9px] text-body sm:text-[10px] lg:text-[11px]">Vedic Astrologer</p>
            <span className="mt-3 inline-flex rounded-btn bg-gold px-3 py-2 text-[9px] font-semibold leading-none text-white sm:px-3.5 sm:text-[10px]">
              Book<span className="ml-1 hidden sm:inline">Consultation</span>
            </span>
          </div>

          <div className={`shrink-0 overflow-hidden rounded-[7px] bg-[var(--color-peach)] ${UI.browser.imageWidth} ${UI.browser.imageHeight}`}>
            <img src={photo} alt={name} className="h-full w-full object-cover object-top" />
          </div>
        </div>
      </div>

      <PhonePreview tint={tint} photo={photo} name={name} />
    </div>
  );
}
...
function PhonePreview({ tint, photo, name }: { tint: Tint; photo: string; name: string }) {
  const panelBg = tint === "lavender" ? "bg-[var(--color-card-lavender)]" : "bg-[var(--color-card-mint)]";

  return (
    <div className={`absolute z-20 overflow-hidden rounded-[17px] border-navy bg-white shadow-card ${UI.phone.width} ${UI.phone.border} ${UI.phone.right} ${UI.phone.bottom}`}>
      <div className={`overflow-hidden rounded-[13px] ${panelBg}`}>
        <div className={`w-full overflow-hidden ${UI.phone.imageHeight}`}>
          <img src={photo} alt={name} className="h-full w-full object-cover object-top" />
        </div>

        <div className={`flex flex-col gap-1.5 px-2 py-2 ${UI.phone.height}`}>
          <div>
            <p className="text-[8px] font-semibold leading-tight text-navy">{name}</p>
            <p className="text-[6px] leading-tight text-body">Vedic Astrologer</p>
          </div>

          <div className="flex items-center gap-1">
            <span className="text-[7px] text-gold">★</span>
            <span className="text-[6px] font-medium text-navy">4.9</span>
            <span className="text-[6px] text-body">• 120+ sessions</span>
          </div>

          <div className="rounded-md bg-white/70 px-1.5 py-1">
            <p className="text-[5.5px] text-body">Next available</p>
            <p className="text-[6.5px] font-semibold text-navy">Today · 7:30 PM</p>
          </div>

          <div className="flex items-center justify-between rounded-md bg-white/60 px-1.5 py-1">
            <div>
              <p className="text-[5.5px] text-body">Consultation</p>
              <p className="text-[6.5px] font-semibold text-navy">30 min</p>
            </div>
            <p className="text-[7px] font-semibold text-navy">₹999</p>
          </div>

          <div className="mt-auto flex h-5 w-full items-center justify-center rounded-btn bg-gold sm:h-6">
            <span className="text-[6.5px] font-semibold text-white">Book Consultation</span>
          </div>
        </div>
      </div>
    </div>
...
function BillingToggle({ value, onChange }: { value: BillingPeriod; onChange: (value: BillingPeriod) => void }) {
  return (
    <div className={`mx-auto flex ${UI.toggle.width} ${UI.toggle.height} rounded-full border border-[#cdd3df] bg-white p-1 shadow-sm`}>
      <button
        type="button"
        onClick={() => onChange("monthly")}
        className={`flex flex-1 items-center justify-center rounded-full ${UI.toggle.textSize} font-medium transition-all ${value === "monthly" ? UI.toggle.active : UI.toggle.inactive}`}
      >
        Monthly
      </button>
      <button
        type="button"
        onClick={() => onChange("annually")}
        className={`flex flex-1 items-center justify-center rounded-full ${UI.toggle.textSize} font-medium transition-all ${value === "annually" ? UI.toggle.active : UI.toggle.inactive}`}
      >
        Annually
      </button>
    </div>
...
function PlanCard({ plan, billingPeriod, onOpenDemo }: PlanCardProps) {
  const isLavender = plan.tint === "lavender";
  const billing = plan[billingPeriod];
  const badgeClass = isLavender ? "bg-[var(--color-lavender-bg)] text-lavender" : "bg-[var(--color-sage-bg)] text-sage";

  return (
    <article className={`relative flex min-w-0 flex-col overflow-visible rounded-[16px] border border-border bg-white ${UI.card.minHeight} ${UI.card.padding}`}>
      <div className={`grid min-w-0 flex-1 grid-cols-1 ${UI.card.columns} ${UI.card.innerGap}`}>
        <div className="flex min-w-0 flex-col">
          <div>
            <span className={`inline-flex max-w-full rounded-full ${UI.badge.padding} ${UI.badge.text} font-medium leading-none ${badgeClass}`}>
              {plan.badge}
            </span>
          </div>

          <h3 className={`font-heading font-semibold leading-none text-navy ${UI.title.size} ${UI.title.marginTop}`}>{plan.title}</h3>
          <p className={`max-w-[220px] leading-[1.35] text-body ${UI.subtitle.size} ${UI.subtitle.marginTop}`}>{plan.subtitle}</p>

          <div className={`flex items-baseline gap-1.5 ${UI.price.marginTop}`}>
            <span className={`font-heading font-semibold leading-none text-navy ${UI.price.size}`}>{billing.price}</span>
            <span className={`text-body ${UI.price.unitSize}`}>{billing.unit}</span>
          </div>

          <ul className={`mt-auto pt-5 ${UI.features.gap}`}>
            {plan.features.map((feature) => (
              <CheckItem key={feature} label={feature} tint={plan.tint} />
            ))}
          </ul>

          <div className={`flex flex-col sm:flex-row lg:flex-col xl:flex-row ${UI.buttons.gap} ${UI.buttons.marginTop}`}>
            <button
              type="button"
              className={`inline-flex ${UI.buttons.height} ${UI.buttons.primaryWidth} items-center justify-center whitespace-nowrap rounded-btn px-4 font-semibold text-white transition-opacity hover:opacity-90 ${UI.buttons.text} ${isLavender ? "" : "bg-sage"}`}
              style={isLavender ? { backgroundImage: "var(--gradient-button)" } : undefined}
            >
              {plan.primaryLabel} →
            </button>
        ...
        <div className="flex min-w-0 items-center">
          <div className="w-full">
            <BrowserPreview url={plan.url} tint={plan.tint} photo={plan.photo} name={plan.astrologerName} />
          </div>
        </div>
      </div>
    </article>
...
export default function TwoWaysToGrow() {
  const [openDemo, setOpenDemo] = useState<DemoKey>(null);
  const [billingPeriod, setBillingPeriod] = useState<BillingPeriod>("monthly");

  return (
    <section
      className={`relative overflow-hidden ${UI.section.paddingY}`}
      style={{ background: "var(--gradient-section-bg)" }}
    >
      {/* Shared brand geometry — same motif system as the hero,
          kept extremely subtle so it reads as texture, not decoration. */}
      <KundaliDiamond className="-right-20 -top-20 h-[420px] w-[420px]" opacity={0.04} />
      <GatewayMotif className="-left-24 bottom-0 h-[380px] w-[380px]" opacity={0.04} />

      <div className={`container-page relative mx-auto ${UI.section.maxWidth}`}>
        <div className="flex flex-col items-center text-center">
          <h2 className={`font-heading font-semibold leading-[1.05] text-navy ${UI.heading.size}`}>Two Ways to Grow</h2>
          <p className={`${UI.heading.descriptionWidth} ${UI.heading.descriptionMargin} ${UI.heading.descriptionSize} leading-[1.55] text-body`}>
            Whether you&apos;re just starting or ready for a fully customised platform, we have the right solution for your astrology business.
          </p>
        ...
        <div className={`grid grid-cols-1 lg:grid-cols-2 ${UI.section.cardsGap} ${UI.section.cardsTopMargin}`}>
          <PlanCard plan={plans.starter} billingPeriod={billingPeriod} onOpenDemo={() => setOpenDemo("starter")} />
          <PlanCard plan={plans.custom} billingPeriod={billingPeriod} onOpenDemo={() => setOpenDemo("custom")} />
        </div>
      </div>

      <DemoPreviewModal isOpen={openDemo === "starter"} onClose={() => setOpenDemo(null)} tint="lavender" name="Rahul Sharma" title="Vedic Astrologer" url="rahulsharma.yourplatform.com" />
      <DemoPreviewModal isOpen={openDemo === "custom"} onClose={() => setOpenDemo(null)} tint="sage" name="Jyotish Acharya" title="Vedic Astrology & Life Guidance" url="www.yourdomain.com" />
    </section>
  );
}